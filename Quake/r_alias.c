...
void R_FlushAliasInstances (qboolean showtris)
{
	...
	for (hdr = mainhdr; hdr; hdr = hdr->nextsurface ? (aliashdr_t*)((byte*)hdr + hdr->nextsurface) : NULL)
	{
		...
		if (!translucent && (strstr (textures[0]->name, "_flash") || strstr (textures[0]->name, "_alpha")))
			continue;
	...
	if (!translucent)
	{
		...
		for (hdr = mainhdr; hdr; hdr = hdr->nextsurface ? (aliashdr_t*)((byte*)hdr + hdr->nextsurface) : NULL)
		{
			...
			if (!(strstr (textures[0]->name, "_flash") || strstr (textures[0]->name, "_alpha")))
			{ 
				continue;// Not a flash
			}
