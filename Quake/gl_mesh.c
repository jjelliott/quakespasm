...
void GLMesh_LoadVertexBuffer(qmodel_t* m, aliashdr_t* mainhdr)
{
	...
	for (hdr = mainhdr, numverts = 0, numindexes = 0; ;)
	{
		...
		{
			meshst_t* st; float hscale, vscale;
			hscale = 1.0f / (float)TexMgr_PadConditional (hdr->skinwidth); vscale = 1.0f / (float)TexMgr_PadConditional (hdr->skinheight);
			hdr->vbostofs = stofs;
			st = (meshst_t*)(vbodata + stofs);

			stofs += hdr->numverts_vbo * sizeof (meshst_t);
	...
	m->meshindexesvbo = GL_CreateBuffer(GL_ELEMENT_ARRAY_BUFFER, GL_STATIC_DRAW, va("%s indices", m->name),
